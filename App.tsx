...
const NavItem: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`flex items-center space-x-3 px-3 py-2.5 rounded-md text-sm font-medium w-full transition-colors ${
      isActive
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 font-sans flex">
      <aside className="w-64 bg-slate-900/70 backdrop-blur-lg border-r border-slate-800 flex-shrink-0 flex flex-col p-4">
        <div className="flex items-center space-x-3 p-3 mb-6">
           <svg className="w-8 h-8 text-cyan-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            ...
            <h1 className="text-xl font-bold text-slate-100">System Suite</h1>
        </div>
