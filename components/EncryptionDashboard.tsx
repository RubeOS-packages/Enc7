...
const EncryptionDashboard: React.FC<EncryptionDashboardProps> = ({ onNavigate }) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      const newFiles: FileWithProgress[] = Array.from(event.target.files).map(file => ({ file, progress: 0 }));
  ...
  const handleEncrypt = useCallback(async () => {
    ...
    try {
      const filePromises = files.map(async (fileWithProgress, index): Promise<VaultFile> => {
        ...
        const data = await fileToBase64(fileWithProgress.file, onProgress);
        return { name: fileWithProgress.file.name, type: fileWithProgress.file.type, data };
      });
